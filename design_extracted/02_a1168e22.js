    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', background: 'var(--panel)', borderRight: '1px solid var(--line)' }}>
      ...
      <div style={{ padding: '14px 14px 10px', flexShrink: 0 }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 10 }}>
          ...
          <button style={{ display: 'flex', alignItems: 'center', gap: 3, fontSize: 11, color: 'var(--ink-3)', padding: '3px 6px', borderRadius: 5 }}>
            <I.plus size={12}/> New
          </button>
        </div>
...
const StatusFooter = ({ count }) => {
  ...
  const statuses = [
    { k: 'backend', label: 'Backend',       sub: 'api.loom.ai', state: 'ok' },
    { k: 'assets',  label: 'Asset library', sub: 'S3 · 1.2k',    state: 'ok' },
    { k: 'model',   label: 'Model',          sub: 'SDXL-turbo',   state: 'warn' },
  ];
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        {statuses.map(s => <StatusRow key={s.k} {...s}/>)}
        ...
        <div style={{
          ...
          <span className="mono">{count} templates</span>
          <button style={{ display: 'flex', alignItems: 'center', gap: 4, color: 'var(--ink-2)' }}>
            <I.folder size={11}/> My library
          </button>
        </div>
      </div>
    </div>
