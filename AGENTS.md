...
The backend serves the frontend as static files at `/ui`. There is no Node.js build step — `frontend-dist/index.html` contains inline `<script type="text/babel">` blocks that are compiled in the browser. `frontend-dist/build.py` is known broken (see IDEAS.md #10).
...
- **No frontend build step**: React runs in the browser via Babel standalone. JSX files in `frontend-dist/src/` are inlined into `index.html` via `build.py`.
